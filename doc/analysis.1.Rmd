...
metadata <- tibble(
  ...
  mutate(group = relevel(group, "TregRest")) %>%
  as.data.frame
```
...
```{r read_glycogenes}
file <- paste0(dirs$data, "glycogenes.txt")
...
refseq <- unique(as.character(glyco$yourlist))
refseq <- unlist(strsplit(refseq, ","))
```

It contains `r N(nrow(glyco))` entires with `r N(length(refseq))` unique RefSeq identifiers. This is a result of mapping RefSeq to UniProt. I use Ensembl to map RefSeq IDs into Ensembl IDs used in this work.
...
```{r glyco_ens}
myKable(glyco.map) %>% scroll_box(height = "400px", width = "400px")
```

This gives us `r N(nrow(glyco.map))` mappings with `r N(length(glyco.ens))` unique Ensembl IDs associated with glycogenes.
...
plotStarSense(paste0(dirs$readcount, "TregRest_1.txt"))
```
...
The strong peak on the right is the start of MT, so probably not interesting. The peak on the left is in chromosome 1, bin 24.0-24.5 Mb. We might check it later to see what gene it is.
...
Note that there seem to be differences between replicates 1-2 and 3-4 in both conditions. Sequencing was done in one go, on a single flow cell. Perhaps this is due to how samples were prepared?
...
save(DE, expr, go.terms, gene.go, reactome.terms, gene.reactome, metadata, glyco.ens, file="../RData/shiny_data.RData")
rm(DE, expr)
```
...
Out of the `r length(glyco.ens)` glycogenes we have `r length(glyco.sel)` genes that are differentially expressed. Here I show intensity plots for all of them.
